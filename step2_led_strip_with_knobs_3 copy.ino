/*
  5-band musical role mapping -> WS2812 LED strip
  + 3-knob live band tuning (jump-free relative scaling)
  + OLED display + knob-based tuning on/off
  ----------------------------------------------------------
  Band roles, repeating every 5 pixels down the strip:
    pixel i%5==0: Kick/808         - onset flash
    pixel i%5==1: Snare/other beat - onset flash
    pixel i%5==2: Cymbals/synths   - onset flash
    pixel i%5==3: Vocals (approx)  - continuous glow
    pixel i%5==4: Instruments (approx) - continuous glow

  TUNING MODE: no separate switch needed. Turning the band-select knob
  all the way down (below a low-end deadzone) puts the system in "off"
  mode - no band selected, the other two knobs are ignored, bands stay
  frozen at whatever they were last set to, and the OLED's ">" selector
  disappears. Turning it up past that deadzone re-enters tuning mode
  and selects among the 5 bands, using relative/reference-point scaling
  so switching which band you're editing doesn't cause a value "jump."

  Wiring additions beyond mic/ESP32/level shifter/LED strip:
    Potentiometer (x3), 10k linear taper each:
      outer leg 1 -> 3.3V, outer leg 2 -> GND, wiper -> knob pin below
      0.1uF cap from each wiper to GND, close to the ESP32 pin

    OLED SSD1306 (I2C, 128x64):
      VCC -> 3.3V, GND -> GND, SDA -> board SDA, SCL -> board SCL

  Libraries required: ArduinoFFT (v2.x API), FastLED,
                       Adafruit_SSD1306, Adafruit_GFX, Wire (built-in)
*/
...
// Forward declarations - works around an Arduino IDE quirk where it
// auto-generates function prototypes and inserts them above custom
// struct definitions, breaking compilation.
struct OnsetBand;
...
// hue offsets (FastLED 0-255 hue scale) from the base color, spread evenly
// around the wheel (multiples of 255/5=51) so the 5 bands look visually
// distinct rather than clustered near one hue
const int8_t hueOffsets[5] = {-25, -10, 0, 10, 25};
uint8_t currentBaseHue = 160; // seed value (blue-ish), overwritten by the color knob
...
// ---- Mode switch: beat sync vs constant ambient ----
#define MODE_SWITCH_PIN 26
bool ambientMode = false; // switch open (default) = beat sync, closed = ambient
const uint8_t AMBIENT_BRIGHTNESS = 180; // TUNE ME - fixed brightness for ambient mode

// ---- Knob config ----
// TUNE ME: adjust to actual free ADC1-capable pins on your specific board
...
// ---- Band definitions (bin index ranges) - your tuned defaults ----
...
// ---- Live-tunable band ranges (index 0-4: kick,snare,cymbal,vocal,instr) ----
...
const bool DEBUG_PRINT_LEVELS = true;
...
// =========================================================
// Knob reading + live band retuning (jump-free via relative scaling)
// =========================================================
float bandSelectSmooth = 0, startKnobSmooth = 0, endKnobSmooth = 0;
const float knobAlpha = 0.2;

// bottom slice of the band-select knob's range = "off" (no band selected,
// knobs ignored entirely). TUNE ME: raise if it triggers too easily near
// the knob's true zero, lower if "off" is hard to reach.
const float OFF_ZONE_THRESHOLD = 200;
...
void readKnobsAndRetune() {
  ...
  if (bandSelectSmooth < OFF_ZONE_THRESHOLD) {
    // off mode - nothing gets edited, bands stay exactly where they were
    currentSelectedBand = -1;
    lastSelectedBand = -1;  // forces a fresh reference capture next time a band is selected
    return;
  }

  // map the remaining knob range (above the off deadzone) into 5 even zones
...
// TUNE ME: the ESP32 ADC often doesn't reach true 0/4095 at a knob's
// physical extremes. Find your actual min/max with DEBUG_PRINT_LEVELS,
// turning the knob fully each direction, then set these accordingly -
// this is what makes the hue actually cycle through the FULL wheel.
...
// =========================================================
// Percussion (onset/flash) bands - YOUR tuned values
// =========================================================
struct OnsetBand {
  int startBin, endBin;
  int outIndex;
...
// =========================================================
// Vocal / instrument (continuous) bands - YOUR tuned values
// =========================================================
struct ContinuousBand {
  int startBin, endBin;
  int outIndex;
...
// =========================================================
void updateDisplay() {
  // throttle - the display doesn't need to redraw 30-40 times a second,
  // and every extra I2C transaction is a chance for a bus glitch to hang
